...
// John Doe -> JD
// ✅ Blindado contra undefined, null o strings vacíos
export function getInitials(name) {
  if (!name || typeof name !== "string") return "?"; // Devuelve un signo de interrogación si no hay nombre
  ...
  return name
    ...
    .join("")
    .toUpperCase(); // De paso lo aseguramos en mayúsculas (ej: jd -> JD)
}
...
function mapUserToConversation({ user, messages, authUser, onlineUsers }) {
  ...
  return {
    id: user._id,
    peer: {
      name: user.fullName || "Usuario Anónimo", // 👈 Salvavidas si no hay fullName
      subtitle: user.email || "Sin email",       // 👈 Salvavidas si no hay email
